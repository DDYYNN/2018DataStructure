...
/**
 * command 입력을 공통적으로 처리하는 클래스.
 * 파일 입력, 슬롯 출력, 패턴 검색을 담당한다.
 *
 */
...
//////////////////////////////////////////////////////////////////////////////
	...
	private void parseEachLine(String line, int strIdx){
		for(int charIdx = 0; charIdx <= line.length() - PATTERN_LENGTH; charIdx++){
			...
			IndexPair p = new IndexPair(strIdx, charIdx + 1);		// charIdx는 1에서부터 시작.
			DB.put(s,p);
		}
	}
...
//////////////////////////////////////////////////////////////////////////////
	 ...
	 * <p> {@code input}을 6개의 문자로 구성된 substring들로 쪼개서
	 * 각 substring마다 매칭되는 index list들을 찾은 후,
	 * 각 index가 하나의 pattern으로 연결되어 있는지를 확인한다.
	 * 
	 * @param input
	 * @return List of all indexes where {@code input} pattern appears.
	 * If no index exists, return a {@code dumbList} : (0, 0)
	 *
	 */
	...
	private ArrayList<IndexPair> matchingList(String input){
		...
		int n = input.length() - PATTERN_LENGTH + 1;	// input은 n개의 6-character substring으로 쪼개진다.
		...
		for(IndexPair p : partiallyMatchedList.get(0)){	// p : pattern이 시작된다고 예상되는 위치들
			...
			for(int i = 1; i < n; i++){					// 모든 input의 substring들에 대하여
				...
				if(!adjList.contains(adjacency))		// (i,j)가 매칭되면 (i,j+1)도 매칭되어야 한다.
					isFullyMatched = false;
			}
			if(isFullyMatched)
				fullyMatchedPairs.add(p);				// 모든 n개의 substring이 다 matching되는 index만 저장.
		}
		
		return fullyMatchedPairs.isEmpty() ? dumbList : fullyMatchedPairs;	 // 하나도 없으면 dumbList 반환.
	}
}
