 ...
 *	<li> modification from <b><i>Data Abstraction & Problem Solving with JAVA: 
 *	Walls and Mirrors</i>, J.J.Prichard, 3rd ed.</b>
 *	
 *
 */		 
...
//////////////////////////////////////////////////////////////////////////////
	...
	private AVLTreeNode<T,P> insertItem(AVLTreeNode<T,P> tNode, T newItem,
			P pair){
	...
	/**
	 * {@code tNode}가 어느 불균형 case에 있는지를 판단한다.
	 * 왼쪽으로 치우치면("L case") 2, 오른쪽으로 치우치면("R case") -2를 반환하며,
	 * 나머지(-1,0,1)의 값의 경우 균형이 맞음을 의미한다.
	 * 
	 * @return (height of {@code tNode}'s left subtree)
	 * 			- (height of {@code tNode}'s right subtree) = -2, -1, 0, 1, 2
	 * 
	 */
	...
	private AVLTreeNode<T,P> L_Rotate(AVLTreeNode<T,P> tNode){
		...
		R.setLeft(tNode);
		tNode.setRight(RL);
		updateHeight(tNode);	updateHeight(R);
	...
	private AVLTreeNode<T,P> R_Rotate(AVLTreeNode<T,P> tNode){
		...
		L.setRight(tNode);
		tNode.setLeft(LR);
		updateHeight(tNode);	updateHeight(L);
...
//////////////////////////////////////////////////////////////////////////////
	...
	private String printPreOrder(AVLTreeNode<T,P> start){
		...
		return " " + start.getItem().toString()
				+ printPreOrder(start.getLeft()) + printPreOrder(start.getRight());
	}
...
//////////////////////////////////////////////////////////////////////////////
	...
	public LinkedList<P> searchList(T searchKey){
		...
		if(found == null) return null;		// if not found, return null
		return found.getList();
	}
	
	private AVLTreeNode<T,P> searchNode(AVLTreeNode<T,P> curr, T searchKey){
		if(curr == null) return null;		// if not found, return null
		else{
