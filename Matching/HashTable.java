...
public class HashTable {
	...
	/**
	 * {@code item}을 hashing한 값({@code hashCode})에 따라 hash table에 {@code item}을
	 * 저장한다. hash table은 {@link AVLTree}를 담는 배열이므로, 실제로는 {@code String item}과
	 * {@link IndexPair} {@code idx}를 입력받아 {@code AVLTree}에 insert하는 역할을 한다.
	 */
	public void put(String item, IndexPair idx) {
		int hashCode = hashValue(item); // hashing item
		if (items[hashCode] == null) { // if slot is empty,
			AVLTree<String, IndexPair> avl = new AVLTree<>(); // make a new AVL
																// tree
			avl.insert(item, idx);
			items[hashCode] = avl;
		} else {
	 ...
	 * 
	 * @param searchKey
	 * @return {@code LinkedList<P> list} : Index list of treenode with
	 *         {@code searchKey}
	 */
