...
/**
 * AVL 트리의 generic node. 각 노드에는 substring과, substring의 index들이 들어있는
 * {@code LinkedList}가 저장된다.
 *
 * @param <T>
 *            <b>{@code extends Comparable}</b> Item of node ({@code String}
 *            substring)
 * @param <P>
 *            Pair that goes into the node's {@code LinkedList}
 *            ({@link IndexPair} pair)
 */
public class AVLTreeNode<T extends Comparable<T>, P> {
	...
	public AVLTreeNode(T newItem, P pair, AVLTreeNode<T, P> leftChild, AVLTreeNode<T, P> rightChild) {
		item = newItem;
