...
//					System.out.println((System.nanoTime() - nt) + " ns");
//				}
//				else
//				{
...
//
//			}
		for(int numsize = 100000; numsize < 1000000; numsize += 100000){
			long[] Bt = new long[3];
			long[] It = new long[3];
			long[] Rt = new long[3];
			for(int run = 0; run < 3; run++){
				Random rand = new Random();
				int[] value = new int[numsize];	// 배열을 생성한다.
				int rmaximum = numsize; int rminimum = -numsize;
				for (int i = 0; i < value.length; i++)	// 각각의 배열에 난수를 생성하여 대입
					value[i] = rand.nextInt(rmaximum - rminimum + 1) + rminimum;
				...
				for(int j = 0; j < 3; j++){
					...
					newvalue = DoHeapSort(newvalue);
					Bt[run] = System.currentTimeMillis() - bt;
				}
				
				for(int j = 0; j < 3; j++){
					int[] newvalue2 = (int[])value.clone();
					long it = System.currentTimeMillis();
					newvalue2 = DoMergeSort(newvalue2);
					It[run] = System.currentTimeMillis() - it;
				}
				
				for(int j = 0; j < 3; j++){
					int[] newvalue3 = (int[])value.clone();
					long rt = System.currentTimeMillis();
					newvalue3 = DoQuickSort(newvalue3);
					Rt[run] = System.currentTimeMillis() - rt;
				}
			}
			System.out.println("numsize = " + numsize);
			System.out.println("Heap : average = " + avg(Bt) + ", stdev = " + stdv(Bt));
			System.out.println("Merge : average = " + avg(It) + ", stdev = " + stdv(It));
			System.out.println("Quick : average = " + avg(Rt) + ", stdev = " + stdv(Rt));
		}
	}
	...
	private static double avg(long[] arr3){
		return (arr3[0] + arr3[1] + arr3[2])/3;
	}
	
	private static double stdv(long[] bt){
		double result = 0;
		for(int i = 0; i < 3; i++){
			result += Math.pow(bt[i] - avg(bt), 2);
		}
